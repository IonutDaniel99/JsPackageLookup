...
function App() {
  ...
  return (
    <>
      <Show when={rect().width < 1024}>
        <div class="absolute h-screen w-screen top-0 left-0 bg-gray-900 z-20 text-white flex justify-center items-center">
          <p class="font-bold text-2xl text-center ">Please remove zoom or increase browser width.</p>
        </div>
      </Show>
      <div class="h-screen w-screen flex justify-center bg-gray-900">
        <div class="m-4 w-full px-10 bg-gray-800 rounded-xl 2xl:mx-auto 2xl:container 2xl:w-4/6  ">
          <header>
            <nav class="py-4">
              <ul class="flex items-center justify-between gap-4 text-white">
                <img class="h-12 w-fit" src={Logo} height={48} width={240} alt="JsPackageLookup Logo" />
                <div class="flex items-center justify-start gap-4 pb-2">
                  <li>
                    <a class="font-semibold text-lg hover:underline underline-offset-8 group" href="https://status.npmjs.org/" target="_blank">
                      <span class="group-hover:text-rose-500">NPM</span> Status
                    </a>
                  </li>
                  <li class="scale-150">
                    <span class="relative px-1 bottom-[2px] text-gray-500">•</span>
                  </li>
                  <li>
                    <span class="font-semibold text-lg hover:underline underline-offset-8 cursor-pointer" onClick={() => openModal()}>
                      About
                    </span>
                  </li>
                </div>
          ...
          <main class="flex pt-10 relative h-5/6">
            <aside class="w-3/12 mr-4 relative">
              <InputZone sendJsonToParent={handleJsonFromInputZone} />
            </aside>
            <section class="w-9/12 flex flex-col gap-2 pt-7">
        ...
        <Transition appear show={isOpen()}>
          <Dialog isOpen class="fixed inset-0 z-10 overflow-y-auto flex items-center justify-center h-screen w-screen" onClose={closeModal}>
                          ...
                          return (
                            <>
                              ...
                              <ul class="list-disc ml-8 pb-2">
                                <For each={data}>{(version, z) => <li>{version}</li>}</For>
                              </ul>
                            </>
                          );
                        }}
                    ...
                    <div>
                      <div class="w-full flex items-center justify-center mb-4">
                        <p class="text-sm">
                          Created by{" "}
                          <a
                            href="https://github.com/IonutDaniel99"
                            class="text-blue-500 font-semibold underline underline-offset-4 outline-offset-0 outline-0 focus-visible:outline-offset-0"
                            target="_blank"
                          >
                            Ionut Daniel (GitHub)
                          </a>
